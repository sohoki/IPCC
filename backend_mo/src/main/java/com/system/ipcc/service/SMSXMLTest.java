...
public class SMSXMLTest {
	...
	*/
	...
	private void releaseSession() {
		...
		log.error("The SMS session was released.  result_code=" + result.getReturn().getResultCode());
	}
								...
								if (value != null) {
										if( vType.getName().equals("java.lang.String") )
											...
											for(int k=0; k < valueAL.size(); k++){
												...
												if(v != null)
												 log.error(vName + "[" + v.getPosition() +"] = " + '"'   + v.getValue() +'"');
	...
	public boolean loadProps() {
		...
		try {
			...
			log.error("The XML in SubmitRequest.xml in invalid and can not be unmarshalled!!");
			return false;
		} catch (ClassCastException cce) { 
			// A fault was raised. The fault message will contain the explanation
			log.error(cce.getMessage());
			log.error("Please verify that the modelFields element contains a valid SMS Model!");
			return false; 
		} catch (IllegalArgumentException arge) { 
			// A fault was raised. The fault message will contain the explanation
			log.error(arge.getMessage());
			log.error("Please verify that SubmitRequest.xml and smsxml.properties files exist " +
